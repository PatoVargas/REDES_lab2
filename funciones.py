...
"""
def bandpass(lowcut, highcut, rate, order=5):
    rate_medio = 0.5 * rate
    inferior = lowcut / rate_medio
    superior = highcut / rate_medio
    b, a = butter(order, [inferior, superior], btype='band')
    return b, a
...
Funcion: realizar el filtro a la señal original. Requiere de los servicios de bandpass
"""
def filtro(data, lowcut, highcut, rate, order=5):
    b, a = bandpass(lowcut, highcut, rate, order=order)
    y = lfilter(b, a, data)
    return y
...
def aplicarFiltro(lowcut,highcut,signal,rate):
	...
	frq = k/T #Se asigna una frecuencia a cada elemento de k
	y = filtro(signal, lowcut, highcut,rate, order=1)
	plt.plot(frq, y)
	plt.grid(True)
	plt.axis('tight')
